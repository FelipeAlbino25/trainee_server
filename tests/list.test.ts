...
describe('POST api/v1/lists', async () => {
  ...
  it('POST /api/v1/lists should return 201 and created list', async () => {
    ...
    const response = await request(app).post('/api/v1/lists').send(newList);
...
describe('PUT api/v1/lists/:id', () => {
  it('PUT /api/v1/lists/:id should return 200 and updated list', async () => {
    const newList = await request(app).post('/api/v1/lists').send({ name: 'UpdateTestList0001' });

    const updatedList = {
      id: newList.body.id,
      name: 'Updated Test List',
    };
    const response = await request(app).put(`/api/v1/lists/${updatedList.id}`).send(updatedList);
    expect(response.status).toBe(200);
  });
});
...
describe('DELETE api/v1/lists/:id', () => {
  it('DELETE /api/v1/lists/:id should return 200 and true', async () => {
    const newList = await request(app).post('/api/v1/lists').send({ name: 'DeleteTestList0001' });

    const response = await request(app).delete(`/api/v1/lists/${newList.body.id}`);
    expect(response.status).toBe(200);
...
describe('GET api/v1/lists/:id', () => {
  it('GET /api/v1/lists/:id should return 200 and foundList', async () => {
    const newList = await request(app).post('/api/v1/lists').send({ name: 'FindByIdTestList0001' });

    const response = await request(app).get(`/api/v1/lists/${newList.body.id}`);
    expect(response.status).toBe(200);
