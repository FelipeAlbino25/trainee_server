...
describe('POST api/v1/tasks', async () => {
  ...
  it('POST /api/v1/tasks should return 201 and created task', async () => {
    ...
    const listResponse = await request(app).post('/api/v1/lists').send(newList);
    const newTask = {
      name: 'Test Task0001',
...
describe('PUT api/v1/tasks/:id', async () => {
  ...
  it('PUT /api/v1/tasks/:id should return 200 and updated task', async () => {
    const newList = await request(app)
      .post('/api/v1/lists')
      .send({ name: 'UpdateTestListForTask0001' });
    const newTask = await request(app).post('/api/v1/tasks').send({
    ...
    const response = await request(app).put(`/api/v1/tasks/${newTask.body.id}`).send(updatedTask);
    expect(response.status).toBe(200);
  });
});

describe('DELETE api/v1/tasks/:id', () => {
  it('DELETE /api/v1/tasks/:id should return 200', async () => {
    const newList = await request(app)
      .post('/api/v1/lists')
      .send({ name: 'DeleteTestListForTask0001' });
    const newTask = await request(app).post('/api/v1/tasks').send({
    ...
    const response = await request(app).delete(`/api/v1/tasks/${newTask.body.id}`);
    expect(response.status).toBe(200);
  });
});
...
describe('GET api/v1/tasks/:id', async () => {
  ...
  it('GET /api/v1/tasks/:id should return 200 and foundTask', async () => {
    const newList = await request(app)
      .post('/api/v1/lists')
      .send({ name: 'FindByIdTestListForTask0001' });
    const newTask = await request(app).post('/api/v1/tasks').send({
    ...
    const response = await request(app).get(`/api/v1/tasks/${newTask.body.id}`);
    expect(response.status).toBe(200);
...
describe('GET api/v1/tasks/listId/:listId', () => {
  it('GET /api/v1/tasks/list/:listId should return 200 and tasks for list', async () => {
    const newList = await request(app)
      .post('/api/v1/lists')
      .send({ name: 'FindByListIdTestListForTask0001' });
    const newTask = await request(app).post('/api/v1/tasks').send({
    ...
    const response = await request(app).get(`/api/v1/tasks/listId/${newList.body.id}`);
    expect(response.status).toBe(200);
    expect(Array.isArray(response.body)).toBe(true);
  });
});
